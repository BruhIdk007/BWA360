...
M39/M42 will eventually add the LexMod SDK and the LexMath arithmetic reference mod. M43/M44 later add experimental AR/VR presentation backends. M45 later adds multiplayer/session/network transports. M46 later adds the optional Lexicon Pad custom hardware controller. M47–M50 then cover Avatar/profile presentation, tangible NFC/RFID devices, advanced deterministic gameplay rules and host-bridged BCI research. M2 should **not** implement any of those systems early. Its only obligation is to keep the platform/core layer free of game-specific assumptions such as letters, dictionaries or arithmetic rules, and to keep input/render/platform boundaries clean enough that future Kinect, camera, stereo, HMD, network, profile, physical-device or BCI backends do not require simulation rewrites. Network/profile/device APIs should not be introduced until their actual requirements are validated. Likewise, M2 input should expose normalized buttons/axes/device capabilities rather than assumptions about the future Lexicon Pad, portal or EEG event formats; that hardware/protocol work belongs to the late milestones.
